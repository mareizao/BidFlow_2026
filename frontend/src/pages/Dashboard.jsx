...
const adaptarStats = (backendStats) => {
  ...
  return {
    ...
    abiertas: stats.activas || stats.open || stats.abiertas || 0,
    enRevision: stats.enRevision || stats.review || 0,
    cerradas: stats.completadas || stats.closed || stats.cerradas || 0,
  };
};
...
export default function Dashboard() {
  ...
  // Helper para nombre/rol del usuario
  ...
  // ✅ Verificar si el usuario puede crear licitaciones
  const puedeCrear = usuario?.rol === 'admin' || usuario?.rol === 'pre_sales' ||
                     usuario?.role === 'admin' || usuario?.role === 'pre_sales';
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-gray-800 transition-colors duration-300">
      ...
      <main className="ml-56 pt-16 p-8">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          {/* ✅ Solo mostrar botón si el usuario tiene permiso */}
          {puedeCrear && (
            <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {statsCards.map((s) => (
            <div
              key={s.label}
              className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-5 transition-colors"
            >
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="text-sm font-medium text-slate-900 dark:text-white">
                        {tarea.titulo || tarea.licitacionTitulo || "Sin título"}
                      </h4>
                      <p className="text-xs text-gray-500 dark:text-gray-400">
                        {tarea.descripcion || `Área: ${tarea.area}`}
                      </p>
                    </div>
            ...
            <div className="text-center py-12">
              <p className="text-gray-500 dark:text-gray-400">
                {puedeCrear 
                  ...
                  : "No tienes licitaciones asignadas aún"}
              </p>
              {puedeCrear && (
                <button
                  onClick={() => navigate("/crear")}
                ...
                <thead>
                  <tr className="border-b border-gray-200 dark:border-gray-700 bg-slate-50 dark:bg-gray-800">
                    {["ID", "Nombre", "Estado", "SME Asignado", "Progreso", "Última Actualización"].map((col) => (
                      <th
                        key={col}
                        className="py-3 px-6 text-xs font-semibold tracking-widest text-primary dark:text-blue-400 uppercase"
                      >
                        {col}
                      </th>
                    ))}
                  ...
                  {licitaciones.map((lic, i) => (
                    ...
                    >
                      ...
                      <td className="py-4 px-6">
                        <span
                          className={`px-2 py-1 rounded-full text-xs font-semibold ${badgeEstado(lic.estado || lic.status)}`}
                        >
                        ...
                        <div className="flex items-center gap-2">
                          <div className="flex-1 bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                            <div
                              ...
                              style={{
                                width: `${lic.porcentajeAvance || lic.progreso || lic.progress || 0}%`,
                              }}
                            />
                          </div>
